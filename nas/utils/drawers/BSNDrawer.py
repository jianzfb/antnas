import numpy as np
from nas.utils.drawers.Drawer import Drawer
...
class BSNDrawer(Drawer):
    ...
    @staticmethod
    def get_draw_pos(source=None, dest=None, pos=None, pos_shift=0):
        if pos is not None:
            return pos[0], -pos[1]
        if source is not None and dest is not None:
            src_l, src_s = source
            dst_l, dst_s = dest
            return np.mean([src_l+pos_shift, src_l+pos_shift, dst_l+pos_shift]), -np.mean([src_s+pos_shift, src_s+pos_shift, dst_s+pos_shift])
