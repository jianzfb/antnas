...
# @File    : cityscape_train_and_search.py
...
class CityScapeOutLayer(NetworkBlock):
    n_layers = 1
    n_comp_steps = 1

    def __init__(self, in_chan, out_chan):
        super(CityScapeOutLayer, self).__init__()
        self.conv_1 = nn.Conv2d(in_chan, in_chan, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn_1 = nn.BatchNorm2d(in_chan,
                                   momentum=1.0 if not NetworkBlock.bn_moving_momentum else 0.1,
                                   track_running_stats=NetworkBlock.bn_track_running_stats)
        self.conv_2 = nn.Conv2d(in_chan, out_chan, kernel_size=1, stride=1, padding=0)

        self.params = {
            'module_list': ['CityScapeOutLayer'],
            'name_list': ['CityScapeOutLayer'],
            'CityScapeOutLayer': {'in_chan': in_chan, 'out_chan': out_chan},
            'out': 'outname',
            'in_chan': in_chan,
            'out_chan': out_chan
        }
...
def main(*args, **kwargs):
    ...
    # 获得数据集
    kwargs.update({'img_size': 224, 'in_channels': 3, 'out_channels': 1000, 'task': 'SEGMENTATION'})
    train_loader, val_loader, test_loader, data_properties = \
        get_data(kwargs['dset'], kwargs['bs'], kwargs['path'], kwargs)

    # 创建NAS模型
    nas_manager = Manager(kwargs, data_properties, out_layer=CityScapeOutLayer(192, 2))
    ...
    if len(kwargs['anchor_archs']) > 0:
        anchors = Anchors()
        anchors.load(kwargs['anchor_archs'], kwargs['anchor_states'], CityScapeOutLayer,
                     [int(c) for c in kwargs['cuda'].split(',')])
